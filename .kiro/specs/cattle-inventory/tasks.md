...
- [ ] 1. Configuración inicial del módulo y dependencias
  - [ ] 1.1. Crear estructura de paquetes: mx.vacapp.cattle/ con internal/
  - [ ] 1.2. Actualizar pom.xml con dependencias: Spring Boot, Spring Modulith, JPA, validation
  - [ ] 1.3. Configurar properties en application.yml para cattle module
  - [ ] 1.4. Configurar plugin openapi-generator-maven-plugin para cattle API

- [ ] 2. Crear entidades de dominio puras (domain layer)
  - [ ] 2.1. Crear enum Sex.java (Macho, Hembra)
  - [ ] 2.2. Crear enum Breed.java (Charolais, Angus, Brahman, Hereford, Simmental, Limousin, Criollo, Brangus, Santa Gertrudis, Cruzada)
  - [ ] 2.3. Crear enum CattleStatus.java (Activa, Vendida, Muerta, Prestada, Preñada, En Reposo)
  - [ ] 2.4. Crear enum CattleType.java (Venta, Cría, Engorda, Semental, Vientre)
  - [ ] 2.5. Crear clase AgeCalculator.java con método calculateMonths()
  - [ ] 2.6. Crear clase Animal.java como entidad de dominio inmutable con Builder pattern
  - [ ] 2.7. Implementar métodos de negocio en Animal: isActive(), isSoldOrDead(), markAsSold(), markAsDead(), markAsPregnant()
  - [ ] 2.8. Crear excepciones de dominio: DuplicateAreteException, InvalidGenealogyException, InvalidPastureException, SoldOrDeadAnimalException, AnimalNotFoundException

- [ ] 3. Crear puertos de salida (repository interfaces)
  - [ ] 3.1. Crear interfaz AnimalRepository.java con métodos: save, findById, findByArete, existsByArete, findAll, count, findByStatus, findByRancho
  - [ ] 3.2. Crear interfaz PastureHistoryRepository.java con métodos: insert, findCurrentByAnimalId, updateFechaSalida, findHistoryByAnimalId
  - [ ] 3.3. Crear interfaz WeightRepository.java con métodos: save, findByAnimalId, findPreviousWeight, findLatestWeight
  - [ ] 3.4. Crear interfaz HealthEventRepository.java con métodos: save, findByAnimalId, findByEventType
  - [ ] 3.5. Crear interfaz CattleAuditRepository.java con métodos: logAnimalCreation, logAnimalUpdate, logStatusChange, logMovement

- [ ] 4. Crear entidades JPA y mappers
  - [ ] 4.1. Crear AnimalEntity.java con anotaciones JPA completas
  - [ ] 4.2. Crear PastureHistoryEntity.java con campo calculado dias_permanencia
  - [ ] 4.3. Crear WeightEntity.java
  - [ ] 4.4. Crear HealthEventEntity.java
  - [ ] 4.5. Crear CattleAuditEntity.java
  - [ ] 4.6. Crear AnimalMapper.java para mapear Animal ↔ AnimalEntity
  - [ ] 4.7. Crear PastureHistoryMapper.java
  - [ ] 4.8. Crear WeightMapper.java
  - [ ] 4.9. Crear HealthEventMapper.java

- [ ] 5. Implementar repositorios JPA
  - [ ] 5.1. Crear AnimalJpaRepository extends JpaRepository<AnimalEntity, UUID>
  - [ ] 5.2. Crear PastureHistoryJpaRepository extends JpaRepository<PastureHistoryEntity, UUID>
  - [ ] 5.3. Crear WeightJpaRepository extends JpaRepository<WeightEntity, UUID>
  - [ ] 5.4. Crear HealthEventJpaRepository extends JpaRepository<HealthEventEntity, UUID>
  - [ ] 5.5. Crear CattleAuditJpaRepository extends JpaRepository<CattleAuditEntity, UUID>
  - [ ] 5.6. Crear AnimalRepositoryImpl implements AnimalRepository con filtrado automático por tenant_id
  - [ ] 5.7. Crear PastureHistoryRepositoryImpl implements PastureHistoryRepository
  - [ ] 5.8. Crear WeightRepositoryImpl implements WeightRepository
  - [ ] 5.9. Crear HealthEventRepositoryImpl implements HealthEventRepository
  - [ ] 5.10. Crear CattleAuditRepositoryImpl implements CattleAuditRepository

- [ ] 6. Implementar integración con módulo geográfico
  - [ ] 6.1. Crear GeographyServiceClient.java para integración con geographic-control module
  - [ ] 6.2. Implementar método validatePotreroActive(UUID potreroId) usando GeographyService.isPotreroActive()
  - [ ] 6.3. Implementar manejo de errores cuando potrero no existe o está inactivo

- [ ] 7. Crear comandos y resultados (application layer)
  - [ ] 7.1. Crear CreateAnimalCommand.java con todos los campos del animal
  - [ ] 7.2. Crear UpdateAnimalCommand.java
  - [ ] 7.3. Crear AnimalResult.java con método fromDomain(Animal)
  - [ ] 7.4. Crear MoveAnimalCommand.java con animal_id y potrero_destino_id
  - [ ] 7.5. Crear RecordWeightCommand.java
  - [ ] 7.6. Crear WeightResult.java con ganancia_diaria calculada
  - [ ] 7.7. Crear RecordHealthEventCommand.java
  - [ ] 7.8. Crear HealthEventResult.java

- [ ] 8. Implementar casos de uso de animales
  - [ ] 8.1. Crear CreateAnimalUseCase.java con validación de arete único, validación de potrero, creación de animal, inserción en pasture_history
  - [ ] 8.2. Crear UpdateAnimalUseCase.java con validación de estado (no vendido/muerto)
  - [ ] 8.3. Crear GetAnimalUseCase.java con cálculo de meses en tiempo real
  - [ ] 8.4. Crear ListAnimalsUseCase.java con filtros por status, tipo, rancho, potrero
  - [ ] 8.5. Crear ChangeStatusUseCase.java con validaciones específicas por status (Preñada solo hembras, Vendida con fecha/precio, Muerta con fecha/motivo)
  - [ ] 8.6. Crear SearchAnimalsByAreteUseCase.java con búsqueda case-insensitive

- [ ] 9. Implementar casos de uso de movimientos
  - [ ] 9.1. Crear MoveAnimalUseCase.java con validación de animal activo, validación de potrero destino, actualización de fecha_salida en historial anterior, inserción de nuevo registro
  - [ ] 9.2. Crear GetMovementHistoryUseCase.java con ordenación cronológica, cálculo de dias_permanencia
  - [ ] 9.3. Crear ListAnimalsInPastureUseCase.java para obtener animales actuales en un potrero específico

- [ ] 10. Implementar casos de uso de pesos
  - [ ] 10.1. Crear RecordWeightUseCase.java con validación de animal activo, validación de fecha no futura
  - [ ] 10.2. Crear GetWeightHistoryUseCase.java con ordenación por fecha
  - [ ] 10.3. Crear CalculateDailyGainUseCase.java para calcular ganancia_diaria entre pesos consecutivos

- [ ] 11. Implementar casos de uso de salud
  - [ ] 11.1. Crear RecordHealthEventUseCase.java genérico para vaccination, treatment, diagnosis
  - [ ] 11.2. Crear RecordBirthEventUseCase.java con creación automática de cría, validación de madre hembra, copiado de potrero madre, cambio de status madre si estaba Preñada
  - [ ] 11.3. Crear GetHealthHistoryUseCase.java con ordenación cronológica
  - [ ] 11.4. Crear GetUpcomingVaccinationsUseCase.java para calcular próximas vacunaciones basadas en intervalos

- [ ] 12. Crear DTOs para API REST móvil
  - [ ] 12.1. Crear CreateAnimalRequest.java con validaciones @NotNull, @Size, @Pattern para arete
  - [ ] 12.2. Crear UpdateAnimalRequest.java
  - [ ] 12.3. Crear AnimalResponse.java con campos: animal_id, arete, sexo, raza, edad_meses, tipo, status, potrero_actual, peso_actual, nombre_madre, nombre_padre
  - [ ] 12.4. Crear AnimalDetailResponse.java extendiendo AnimalResponse con genealogía completa
  - [ ] 12.5. Crear MoveAnimalRequest.java
  - [ ] 12.6. Crear PastureHistoryResponse.java
  - [ ] 12.7. Crear RecordWeightRequest.java con validación peso > 0
  - [ ] 12.8. Crear WeightResponse.java con ganancia_diaria
  - [ ] 12.9. Crear RecordHealthEventRequest.java con validación de event_type
  - [ ] 12.10. Crear HealthEventResponse.java
  - [ ] 12.11. Crear CattleStatsResponse.java
  - [ ] 12.12. Crear ErrorResponse.java

- [ ] 13. Crear especificación OpenAPI YAML
  - [ ] 13.1. Crear archivo src/main/resources/openapi/openapi-cattle.yaml
  - [ ] 13.2. Definir schemas para todos los Request/Response DTOs
